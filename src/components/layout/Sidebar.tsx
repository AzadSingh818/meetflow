...
import {
  CalendarDays, LayoutDashboard, Plus, FileText,
  User, LogOut, Bell,
} from 'lucide-react';
...
interface UpcomingMeeting {
  _id: string; title: string; startTime: string; color?: string;
}
      ...
      <aside className={cn(
        ...
        <div className="flex items-center h-16 px-4 border-b border-white/10 flex-shrink-0">
          <Link href="/dashboard" onClick={handleNavClick} className="flex items-center gap-2">
            ...
            <span className={cn('font-bold text-lg tracking-tight', !sidebarOpen && 'md:hidden')}>
              MeetFlow
            </span>
          </Link>
        </div>
        ...
        <div className="border-t border-white/10 p-3 space-y-1 flex-shrink-0">
          ...
          <div ref={notifRef} className="relative">
            <button onClick={() => setNotifOpen(o => !o)}
              ...
              <span className={cn('text-sm', !sidebarOpen && 'md:hidden')}>Notifications</span>
            ...
            <div className="flex items-center gap-3 px-3 py-2.5">
              <div className="w-8 h-8 rounded-full bg-blue-500 flex items-center justify-center flex-shrink-0 overflow-hidden">
                {session.user.image ? (
                  ...
                  <span className="text-xs font-semibold">{getInitials(session.user.name ?? 'U')}</span>
                )}
              </div>
          ...
          <button onClick={() => signOut({ callbackUrl: '/login' })}
            ...
            <span className={cn('text-sm', !sidebarOpen && 'md:hidden')}>Sign Out</span>
          </button>
