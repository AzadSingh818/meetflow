    ...
    <div className="space-y-2">
      ...
      {value.length > 0 && (
        <div className="flex flex-wrap gap-1.5">
          {value.map(a => (
            <span key={a.email}
              className="inline-flex items-center gap-1.5 bg-blue-50 text-blue-800 text-xs font-medium
                         px-2.5 py-1.5 rounded-full border border-blue-200">
              <User className="h-3 w-3" />
              {a.name ?? a.email}
              <button type="button" onClick={() => remove(a.email)}
                className="text-blue-400 hover:text-blue-800 ml-0.5 transition-colors">
                <X className="h-3 w-3" />
              </button>
            </span>
          ))}
      ...
      <div className="flex gap-2">
        <div className="relative flex-1">
          ...
          <input ref={ref} type="email" value={input}
